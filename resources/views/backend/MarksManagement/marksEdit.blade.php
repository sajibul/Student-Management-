...
@section('content')
@section('tittle','Manage  Marks Edit')
@push('css')
<!-- DataTables -->
    ...
    <div class="row">
      ...
      <section class="col-lg-12">
        <div class="card">
          ...
          <div class="card-header">
            <form id="myForm" action="{{route('student-marks-update')}}" method="POST" id="myForm">
              @csrf
...
<script>
  $(document).on('click','#search',function(){
    ...
    $.ajax({
      url:"{{route('get-student-subject-marks')}}",
      type:"GET",
      data:{'year_id':year_id,'class_id':class_id,'assign_subjects_id':assign_subjects_id,'exam_type_id':exam_type_id},
        ...
        $.each(data,function(key, v){
         html +=
          '<tr>'+
            ...
            '<td>'+v.user.fname+'</td>'+
            '<td>'+v.user.gender+'</td>'+
            '<td><input type="text" class="form-control form-control-sm" placeholder="Enter Marks"  name="marks[]" value="'+v.marks+'"></td>'+
          '</tr>';
        });
